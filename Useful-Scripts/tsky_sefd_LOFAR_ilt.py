...
def applyBeamGuassian(grids, temps, plot = False, plothwhm = False, saveplot = False, plotname = None):
	...
	for (frequency, gaussian), temp, grid in zip(gaussians.items(), temps.values(), grids.values()):
		...
		if plot:
			convolvedSky = np.multiply(temp, gaussian)
			#peak = np.where(convolvedSky == convolvedSky.max())
			plt.figure(figsize = (14,10), dpi = 100)
			plt.title(f"Raw Sky Temperatures @\n{frequency:.3g} MHz ({int(temp[grid[0][0].size // 2, grid[1][0].size // 2])}K)")
			plt.xlabel("l [deg]")
			plt.ylabel("b [deg]")
			plt.pcolormesh(grid[0], grid[1], temp)
			plt.colorbar(label = "Temperature [K]")
			plt.scatter(0, 0, alpha = 0.2, c = 'r', s = 4)
			if plothwhm:
				print(hwhm(frequency))
				circ = plt.Circle((0,0), hwhm(frequency), color = 'pink', linestyle = '--', fill = False, linewidth = 4)
				plt.gca().add_patch(circ)
			if saveplot:
				plt.savefig(f"{int(frequency)}MHz_{plotname or np.mean(convolvedSky)}_raw_sky.jpg")
			plt.figure(figsize = (14,10), dpi = 100)
			plt.title(f"Beam-Convolved Sky Temperatures @\n{frequency:.3g} MHz ({int(convTemp[frequency])}K)")
			plt.xlabel("l [deg]")
			plt.ylabel("b [deg]")
			plt.pcolormesh(grid[0], grid[1], convolvedSky)
			plt.colorbar(label = "Contributed Temperature [K]")
			plt.scatter(0, 0, alpha = 0.2, c = 'r')
			if plothwhm:
				circ = plt.Circle((0,0), hwhm(frequency), color = 'pink', linestyle = '--', fill = False, linewidth = 4)
				plt.gca().add_patch(circ)
			if saveplot:
				plt.savefig(f"{int(frequency)}MHz_{plotname or np.mean(convolvedSky)}_convolved_sky.jpg")
			plt.show()


	#print(list(convTemp.keys()), list(convTemp.values()))
...
def getSourceTsky(source, frequencies, model = pygdsm.LowFrequencySkyModel(freq_unit = 'MHz'), sampling = 64, nhwhm = 2, **kwargs):
	...
	pars, convTemp = applyBeamGuassian(grids, temps, **kwargs)
...
def getSEFD_bandavg(args, frequencies, vsamp = 5.):
	...
	source = SkyCoord(args.ra, args.dec, unit = args.units)
	res = getSourceTsky(source, freqs[:, 0].tolist(), model = skyModels[args.model](freq_unit = 'MHz'), sampling = args.samples, nhwhm = args.nhwhm, plot = args.plot, plothwhm = args.plothwhm)
...
if __name__ == '__main__':
	...
	parser.add_argument("--units", '-u', type = str, default = "hourangle, degree", help = "Units to parse for RA/Dec")
	...
	parser.add_argument("--plothwhm", '-L', default = False, action = 'store_true', help = "Whether or not to plot the HWHM is the sky is plotted.")
	...
	parser.add_argument("--saveplot", default = False, action = 'store_true', help = "Save raw and beam-convolved sky plots")
	...
	if args.list:
		...
		for source, (ra, dec) in sources.items():
			src = SkyCoord(ra, dec, unit = args.units)
			results[source] = (getSourceTsky(src, args.freqs, model = model, sampling = args.samples, nhwhm = args.nhwhm, plot = args.plot, plothwhm = args.plothwhm), src)

			print(f"{source}: {np.mean(list(results[source][0][1].values())):.0f}K, {results[source][0][0][1]}")

		averaged = [(key, vals[0][1][150.0]) for key, vals in results.items()]
		averaged.sort(key = lambda x: x[1])
		minS, medianS, maxS = averaged[0], averaged[len(averaged) // 2], averaged[-1]
		print(f"Minimum:\t{minS}")
		print(f"Median:\t{medianS}")
		print(f"Maximum:\t{maxS}")

		averagedA = [(key, vals[0][0][1]) for key, vals in results.items()]
		averagedA.sort(key = lambda x: x[1])
		minA, medianA, maxA = averagedA[0], averagedA[len(averaged) // 2], averagedA[-1]
		print("\n\n")
		print(f"Minimum:\t{minA}, {results[minA[0]][0][-1][110.0] / results[minA[0]][0][1][110.0]}")
		print(f"Median:\t{medianA}, {results[medianA[0]][0][-1][110.0] / results[medianA[0]][0][1][110.0]}")
		print(f"Maximum:\t{maxA}, {results[maxA[0]][0][-1][110.0] / results[maxA[0]][0][1][110.0]}")

		with open(args.output, 'wb') as ref:
			pickle.dump(results, ref)
		cleanResults = {key: val[0] for key, val in results.items()}
		with open(args.output.replace('.pkl', '_clean.pkl'), 'wb') as ref:
			pickle.dump(cleanResults, ref)
		exit()
	else:
		...
		res = getSourceTsky(source, args.freqs, model = skyModels[args.model](freq_unit = 'MHz'), sampling = args.samples, nhwhm = args.nhwhm, plot = args.plot, plothwhm = args.plothwhm, saveplot = args.saveplot)

		print(f"Power law model = {res[0][0]:.5g} * freq **{res[0][1]:.4g}")

		print(f"\n\nFreq [MHz]:\tConv. Temp. [K]\tRaw Temp [K]\tDiff [K]\tDiff [%]")

		for (freq, val), origval in zip(res[1].items(), res[2].values()):
			print(f"{freq}:\t\t{val:.5g}\t\t{origval:.5g}\t\t{val - origval:.3g}\t\t{100 * ((val - origval) / origval):.3g}")
