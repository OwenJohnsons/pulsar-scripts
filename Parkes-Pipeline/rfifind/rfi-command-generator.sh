...
find "${path}" -type f -name "*.sf" | while read -r filename; do
    ...
    if [ -e "${folder_path}/masks/${base_name}_rfifind.sbatch" ]; then
        ...
        if [[ "$base_name" == *"0.7-1.9GHz"* ]]; then
            ...
            echo "rfifind -time 10 -ncpus 4 -noclip -freqsig 8 -timesig 8 -chanfrac 0.6 -intfrac 0.6 -zapchan 0,1493:1571,1913:1991 -o \"${base_name}\" \"$filename\"" >> "$sbatch_script"
        else
